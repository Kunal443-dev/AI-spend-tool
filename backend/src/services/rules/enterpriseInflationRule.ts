...
export const enterpriseInflationRule: AuditRule = {
  ...
  condition: (context) => {
    return context.tools.some(tool => {
      ...
      if (!plan || !plan.metadata?.isEnterprise) return false;
  ...
  recommendation: (context) => {
    ...
    context.tools.forEach(tool => {
      const plan = getPlanDetails(tool.toolId, tool.planId);
      if (!plan || !plan.metadata?.isEnterprise) return;
      
      if (tool.seats < 20) {
        let teamPlanId = 'team';
        if (tool.toolId === 'cursor') teamPlanId = 'business';
        if (tool.toolId === 'copilot') teamPlanId = 'business';
        if (tool.toolId === 'gemini') teamPlanId = 'business';
        
        const teamPlan = getPlanDetails(tool.toolId, teamPlanId);
        if (!teamPlan) return;
