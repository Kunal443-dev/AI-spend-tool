...
if (isSmtpConfigured) {
    ...
    transporterPromise = Promise.resolve(nodemailer_1.default.createTransport({
        host,
        port,
        secure,
        auth: {
    ...
    transporterPromise = nodemailer_1.default.createTestAccount().then(account => {
...
const sendAuditReportEmail = async (email, name, audit) => {
  ...
  `;
    ...
    try {
        ...
        const etherealUrl = nodemailer_1.default.getTestMessageUrl(info);
        if (etherealUrl) {
            console.log(`\n==================================================`);
...
exports.sendAuditReportEmail = sendAuditReportEmail;
